...
首版只支持一个明确的 GitHub Pages 博客模板适配器。统一内容模型由适配器生成 Front Matter、slug、文章目录和资源目录。缺少 `_posts` 的已连接仓库会保留为“待初始化”，作者可确认一个独立的初始化提交，创建 `_posts/` 与资源目录；`.github/easyblog.yml` 仍只在首次内容发布时生成。其他结构问题保留为可恢复状态，不丢弃仓库上下文。

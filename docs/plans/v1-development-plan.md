...
1. **Implement target checks and template adapter** (Release). Complete `backend/src/targets/{target,layout,template,target_check}.rs`. Validate the supported GitHub Pages layout, derive article/resource paths, and generate `.github/easyblog.yml` only on first publish. Add layout mismatch, slug, and generated-file golden tests.
2. **Implement GitHub authorization, repository connection, workspace isolation and diff** (Release). Complete `backend/src/providers/github/auth.rs`, GitHub target connection commands, `backend/src/workspace/{checkout,working_tree,diff,file_lock,commit_log}.rs`, and `backend/src/providers/git/{commands,parser}.rs`. Reuse `gh auth login --web --git-protocol https` with a startup status check; v1 lists current-account repositories with push permission and accepts only `github.com` HTTPS repositories. The user selects `owner/repo`; easyBlog clones it into application data, reuses one target per repository/default branch, and never modifies an existing user clone. Lock one workspace per target, fast-forward only after safe fetch checks, reject dirty/external/ahead states, and expose structured file diffs. Test authorization status/error mapping separately from temporary Git repositories and concurrent lock attempts.
...
4. **Build release and history client flows** (Client). Implement `src/features/releases/index.ts`, `src/features/history/index.ts`, and `src/bridge/{releases,history}.ts` with batch splitting, diff preview, progress, retry, and rollback menu states. Add UI contract tests for blocked items and delete confirmation.
...
5. **Run cross-source integration** (QA). Exercise local, Feishu document, and wiki scopes into one target, split release batches by source/directory, verify independent blocked items, retry a failed subset, and roll back a successful publication.

...
**Tech Stack:** TypeScript, Vite, Vitest, Tauri 2, Rust, SQLite, local Git workspaces, GitHub CLI/API, Feishu APIs, system keychain.
...
2. **Implement workspace isolation and diff** (Release). Complete `backend/src/workspace/{checkout,working_tree,diff,file_lock,commit_log}.rs` and `backend/src/providers/git/{commands,parser}.rs`. Lock one workspace per target, reject dirty/external edits according to the PRD, and expose structured file diffs. Test with temporary Git repositories and concurrent lock attempts.

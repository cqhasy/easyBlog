...
export function bootstrap(root: HTMLElement | null): void {
  ...
  root.innerHTML = `<div class="app-shell"><aside class="app-nav" aria-label="主导航"><div class="app-brand" aria-label="easyBlog">easy<span>Blog</span></div><nav><button type="button" data-page="workbench">工作台</button><button type="button" data-page="sources">来源</button></nav></aside><div class="app-content"><section data-page-panel="workbench"></section><section data-page-panel="sources" hidden></section></div></div>`;
  ...
  root.addEventListener("click", (event) => {
    ...
    if (page === "workbench" || page === "sources") showPage(page);
  });
  showPage("workbench");
}

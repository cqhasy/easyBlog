...
export function renderHistory(records: PublicationRecord[], message = ""): string {
  ...
  return `<section class="history-page" aria-labelledby="history-title"><header class="changes-header"><div><p class="eyebrow">发布记录</p><h1 id="history-title">发布历史</h1><p>已发布提交可生成新的反向提交，不会改写远程历史。</p></div><button type="button" class="task-primary-button" data-action="refresh-history">刷新</button></header>${message ? `<p class="history-message" role="status" aria-live="polite">${escapeHtml(message)}</p>` : ""}<div class="history-column-headings" aria-hidden="true"><span>提交</span><span>目标 / 范围</span><span>状态</span><span>时间</span><span></span></div><ul class="history-list">${rows}</ul>${dialogs}</section>`;
}

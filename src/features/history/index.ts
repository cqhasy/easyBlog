...
export function renderHistory(records: PublicationRecord[], message = ""): string {
  const rows = records.length ? records.map((record) => { const retry = record.state === "pending_push"; const rollbackRetry = record.state === "rollback_pending"; const rollbackAvailable = record.state === "published" && record.rollback_available !== false; const actionable = retry || rollbackRetry || rollbackAvailable; const reason = record.recovery_reason ?? (record.state === "legacy" ? "旧版发布记录没有可安全执行的文件操作清单。" : record.state === "recovery_required" ? "此记录需要恢复处理后才能继续。" : ""); return `<li class="history-row"><div><strong>${escapeHtml(record.commit_sha)}</strong><span>${escapeHtml(record.scope_id)} · ${record.change_ids.length} 项变更</span>${reason ? `<span class="history-unavailable">${escapeHtml(reason)}</span>` : ""}</div><div class="history-meta"><span class="publication-state state-${record.state}">${label(record.state)}</span><time>${escapeHtml(record.published_at ?? "提交已创建，尚未推送")}</time></div><button class="secondary-button" type="button" data-action="${retry ? "retry" : "rollback"}" data-batch-id="${escapeHtml(record.batch_id)}" ${actionable ? "" : "disabled"}>${retry ? "重试推送" : rollbackRetry ? "重试回滚推送" : "回滚"}</button></li>`; }).join("") : `<li class="history-empty">尚无发布记录</li>`;
